...
32bit Address Space(AS)를 4KB 페이지로 나누고, Page Table Entry(PTE)가 4B라면:
...
하지만 Page가 커지면 내부 단편화(internal fragmentation)가 증가해서 비효율적이다.
...
하지만 세그먼트 크기가 크면 여전히 외부 단편화(external fragmentation)가 발생한다.
...
→ 따라서 **VPN(8bit)을 상위 4bit(PD index) + 하위 4bit(PT index)** 로 분할한다.
